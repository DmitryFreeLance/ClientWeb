(() => {
    ...
    const uniqueNamesAged20To30 = _.chain(people)
        .filter(person => person.age >= 20 && person.age <= 30)
        .map("name")
        .uniq()
        .sortBy()
        .reverse()
        .value();
    console.log("Уникальные имена людей с возрастом от 20 до 30:", uniqueNamesAged20To30);

    const namesCount = _.countBy(people, "name");
    console.log("Объект с именами и их количеством:", namesCount);
})();